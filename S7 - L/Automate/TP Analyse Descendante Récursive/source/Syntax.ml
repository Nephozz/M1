  ...
  (match (peekAtFirstToken stream) with
    ...
    | FunctionToken ->
      ...
      (match (accept FunctionToken stream) with
        | Success next -> (match (acceptIdent stream) with
          | Success next2 -> (match (accept BodyToken stream) with
            | Success next3 -> 
              parseE next3 
            | _ -> Failure)
          | _ -> Failure)
    ...
    | IfToken ->
      ...
      (match (accept IfToken stream) with
        | Success next -> (match (parseE next) with
          | Success next2 -> (match (accept ThenToken stream) with
            | Success next3 -> (match (parseE next3) with
              | Success next4 -> (match (accept ElseToken stream) with
                | Success next5 ->
                  parseE next5
                | _ -> Failure)
              | _ -> Failure)
